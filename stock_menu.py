...
def display_chart(stock_list):
    ...
    for stock in stock_list:
        print(stock.symbol, " ", end=" ")
    print("]")
    symbol = input("Choose what stock to chart ").upper()
    found = False
    for stock in stock_list:
        if stock.symbol == symbol:
            found = True
            cur_stock = stock           
    if found == True:
        display_stock_chart(stock_list, cur_stock.symbol)
    else:
        print("Symbol not found")
...
def file_processing(stock_list):
    ...
    choice = ""
    while choice != "E":
        choice = input("Save [S] Load [L] Import [I] Exit [E] ").upper()
        if choice == "S":
            ...
            print("Import data to stock in the stock list")
            symbol = input("Enter symbol ").upper()
